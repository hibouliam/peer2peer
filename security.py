...
from file_share import get_free_port
...
def request_pow_verification(peers, key, my_node, difficulty=4):
    """
    Demande aux pairs voisins de vérifier la validité du PoW.
    """
    nonce = compute_pow(key, difficulty)
    print(f"Demande de vérification PoW : key = {key}, nonce = {nonce}, difficulty = {difficulty}")
    valid_count = 0
    required_validations = max(1, len(peers))  #On demande validation à la majorité des pairs
    ip = my_node[1]
    port=get_free_port()
    print(f"IP : {ip}, Port : {port}")
    print(required_validations, nonce)

    for peer in peers:
        try:
            with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
                s.connect((peer[1], peer[2]))
                message = {"action": "verify_pow", "data" : {"key": key, "nonce": nonce, "difficulty": difficulty, "applicant" : {"ip": ip , "port": port}}}
                s.sendall(msgpack.packb(message))                
                response = json.loads(s.recv(1024).decode())
                print(response.get('valid'))
...
####Ajouter un anti virus api pour vérifier l'intégrité des fichiers